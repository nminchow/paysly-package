...
export default async function Paysly(publicKey) {
  ...
  if (!window.Stripe) {
    ...
    stripeElement.setAttribute('src', 'https://js.stripe.com/v3/');
    document.head.appendChild(stripeElement);
    while(!window.Stripe) {
      await new Promise(resolve => setTimeout(resolve, 0));
    }

    // console.error('It looks like stripe has not been loaded on this page. Please include stripe when loading this package.');
  }
